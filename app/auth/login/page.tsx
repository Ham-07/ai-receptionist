...
import { Bot, ArrowLeft, ShieldCheck, KeyRound } from "lucide-react";
...
function DemoAccountsCard() {
    return (
        <div className="mt-6 p-5 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
                <div className="p-1.5 rounded-lg bg-indigo-500/10 text-indigo-500">
                    <KeyRound className="w-4 h-4" />
                </div>
                <h2 className="text-sm font-bold tracking-tight">Demo Accounts</h2>
            </div>
            <div className="grid sm:grid-cols-2 gap-3">
                {DEMO_ACCOUNTS.map((demo) => (
                    <div
                        key={demo.email}
                        className="p-4 rounded-xl bg-slate-100/70 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60"
                    >
                        <p className="text-sm font-semibold mb-2">{demo.business}</p>
